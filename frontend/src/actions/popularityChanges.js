...
export const getDefaultConfig = () => ({
  ...
  relative: RELATIVITY.NOT_RELATIVE,
  // Defaults to 7-day view if viewed on a weekend, 1 day view otherwise
  hoursAgo: [6, 7].includes(new Date().getDay()) ? 24 * 7 : 24,
  minPopularity: 50,
});

const createQueryParamAdder = key => value => addQueryParam({ [key]: value }, getDefaultConfig());

export const setPopularityChangesChangeType = createQueryParamAdder('changeType');
...
export const setPopularityChangesMinPopularity = createQueryParamAdder('minPopularity');
